...
class RegionalFlexOptimizer:
    ...
    def build_model(self, regional_data: Dict[str, pd.DataFrame]) -> None:
        """Build the optimization model with progress tracking.
        
        Args:
            regional_data: Dictionary of preprocessed regional data
        """
        logger.info(f"Building optimization model for {len(self.regions)} regions")
        
        # Store the regional data for later analysis if needed
        ...
        if isinstance(regional_data, pd.DataFrame):
            logger.warning("Received a DataFrame instead of a dictionary of regional data")
            # Create a dictionary with a single entry for the DataFrame
            timestamp_df = regional_data.copy()
            regional_data = {}
            
            # Check if we have any regions defined
            if not self.regions:
                logger.warning("No regions defined in the optimizer")
                self.model = LpProblem("EmptyModel", LpMinimize)
                self.model += 0, "Empty objective"
                return
        
        # Get time periods from first region's data
        if not self.regions or not regional_data:
            logger.warning("No regions to optimize or empty data provided")
            self.model = LpProblem("EmptyModel", LpMinimize)
            self.model += 0, "Empty objective"
            return
            
        # Extract time periods from the first region's data
        try:
            first_df = next(iter(regional_data.values()))
            time_periods = len(first_df)
        except (StopIteration, AttributeError):
            logger.warning("Could not determine time periods from regional data")
            time_periods = 0
    ...
    def _add_constraints(self, data: Dict[str, pd.DataFrame], time_periods):
        ...
        with tqdm(total=len(constraint_types), desc="Adding constraints", position=1, leave=False) as pbar:
                    ...
                    if len(T) > 0:
                        if f"storage_level_{region}" in self.variables and f"storage_charge_{region}" in self.variables and f"storage_discharge_{region}" in self.variables:
                            self.model += (
                                ...
                                self.variables[f"storage_discharge_{region}"][T[0]] * (1.0 / discharge_efficiency),
                    ...
                    for i in range(1, len(T)):
                        if f"storage_level_{region}" in self.variables and f"storage_charge_{region}" in self.variables and f"storage_discharge_{region}" in self.variables:
                            self.model += (
                                ...
                                self.variables[f"storage_discharge_{region}"][T[i]] * (1.0 / discharge_efficiency),
            ...
            # 6. Ramping constraints - limit how quickly dispatch can change between time periods
            pbar.set_description("Ramping constraints")
            # Only add these constraints if the config includes max_ramp_up and max_ramp_down
            if "max_ramp_up" in self.config["constraints"] and "max_ramp_down" in self.config["constraints"]:
                ...
                for region in self.regions:
                    try:
                        if f"dispatch_{region}" in self.variables:
                            # For each time period except the first
                            for i in range(1, len(T)):
                                # Ramp up constraint: dispatch[t] - dispatch[t-1] <= max_ramp_up
                                self.model += (
                                    self.variables[f"dispatch_{region}"][T[i]] - self.variables[f"dispatch_{region}"][T[i-1]] <= max_ramp_up,
                                    f"ramp_up_{region}_{T[i]}"
                                )
                                
                                # Ramp down constraint: dispatch[t-1] - dispatch[t] <= max_ramp_down
                                self.model += (
                                    self.variables[f"dispatch_{region}"][T[i-1]] - self.variables[f"dispatch_{region}"][T[i]] <= max_ramp_down,
                                    f"ramp_down_{region}_{T[i]}"
                                )
                        
                        # Check for region-specific ramping constraints
                        if "regional_constraints" in self.config and region in self.config["regional_constraints"]:
                            if "max_ramp_up" in self.config["regional_constraints"][region]:
                                region_max_ramp_up = self.config["regional_constraints"][region]["max_ramp_up"]
                                for i in range(1, len(T)):
                                    # Override with region-specific ramp up constraint
                                    self.model += (
                                        self.variables[f"dispatch_{region}"][T[i]] - self.variables[f"dispatch_{region}"][T[i-1]] <= region_max_ramp_up,
                                        f"region_ramp_up_{region}_{T[i]}"
                                    )
                            
                            if "max_ramp_down" in self.config["regional_constraints"][region]:
                                region_max_ramp_down = self.config["regional_constraints"][region]["max_ramp_down"]
                                for i in range(1, len(T)):
                                    # Override with region-specific ramp down constraint
                                    self.model += (
                                        self.variables[f"dispatch_{region}"][T[i-1]] - self.variables[f"dispatch_{region}"][T[i]] <= region_max_ramp_down,
                                        f"region_ramp_down_{region}_{T[i]}"
                                    )
                    except Exception as e:
                        logger.warning(f"Error adding ramping constraints for {region}: {e}")
            else:
                logger.warning("Ramping constraints not added - max_ramp_up or max_ramp_down not found in config")
            pbar.update(1)
    ...
    def solve_model(self) -> Dict:
        ...
        """
        if not self.model or not self.variables:
            logger.error("Model not built. Call build_model first.")
            return None
        
        logger.info("Solving optimization model...")
        
        # Create a progress bar for the solving process
        with tqdm(total=100, desc="Solving model", position=1, leave=False) as pbar:
            solver = self.model.solve(PULP_CBC_CMD(msg=False, timeLimit=3600, gapRel=0.01, options=["timeMode elapsed"]))
        ...
        if self.model.status == LpStatusOptimal:
            ...
            logger.info(f"Found optimal solution with objective value: {value(self.model.objective)}")
            return self.get_results()
        else:
            logger.error(f"Failed to find optimal solution. Status: {status_str}")
            
            # Perform infeasibility analysis if the model is infeasible
            if self.model.status == LpStatusInfeasible:
                infeasibility_report = self._analyze_infeasibility()
                return {"status": "infeasible", "analysis": infeasibility_report}
        ...
        if self.regions and len(self.regions) > 0:
            ...
            if first_var_name in self.variables:
                ...
                logger.warning(f"Variable {first_var_name} not found, using default time periods")
        
        return self.get_results()
        
    def _analyze_infeasibility(self) -> Dict:
        """Analyze the infeasibility to identify problematic constraints.
        
        Returns:
            Dictionary with infeasibility analysis information
        """
        logger.info("Analyzing infeasibility to identify problematic constraints...")
        
        # Create slack variables report structure
        slack_analysis = {
            "slack_variables": {},
            "problematic_regions": [],
        ...
        # Look for slack variables in the model
        for var_name in self.variables:
            if var_name.startswith("slack_"):
                parts = var_name.split("_")
                if len(parts) >= 3:
                    slack_type = parts[1]  # pos or neg
                    region = parts[2]
                    
                    if region not in slack_analysis["slack_variables"]:
                        slack_analysis["slack_variables"][region] = []
                    
                    slack_analysis["slack_variables"][region].append(var_name)
                    if region not in slack_analysis["problematic_regions"]:
                        slack_analysis["problematic_regions"].append(region)
        
        # Check net load values for each region
        for region in self.regions:
            # Add to problematic constraints list based on findings
            constraints_to_check = []
            
            # Check if this region has high net load values that might be problematic
            if region in self.regional_data:
                region_df = self.regional_data[region]
                if "net_load" in region_df.columns:
                    # Get statistics on net load
                    net_load_stats = {
                        "min": region_df["net_load"].min(),
                        "max": region_df["net_load"].max(),
                        "mean": region_df["net_load"].mean(),
                        "extreme_values": len(region_df[region_df["net_load"].abs() > 20000])
                    }
                    
                    slack_analysis["data_analysis"][f"{region}_net_load"] = net_load_stats
                    
                    # Check if net load exceeds capacity constraints
                    max_dispatch = self.config["constraints"]["max_dispatch"]
                    if net_load_stats["max"] > max_dispatch:
                        constraints_to_check.append(
                            f"Net load for {region} exceeds max_dispatch ({net_load_stats['max']} > {max_dispatch})"
                        )
                    ...
                    # Check if there are extreme variations that would require more storage
                    if net_load_stats["max"] - net_load_stats["min"] > storage_capacity * 2:
                        constraints_to_check.append(
                            f"Storage capacity for {region} may be insufficient ({storage_capacity}) compared to net load variation ({net_load_stats['max'] - net_load_stats['min']})"
                        )
            
            # Add exchange capacity constraints check
            exchange_capacity = self.config["constraints"]["exchange_capacity"]
            constraints_to_check.append(
                f"Exchange capacity of {exchange_capacity} MW may be too restrictive for region {region}"
            )
            
            slack_analysis["problematic_constraints"].extend(constraints_to_check)
        
        # Add general infeasibility suggestions
        slack_analysis["suggestions"] = [
            "1. Increase the max_slack parameter to allow more flexibility in the balance constraints",
            "2. Increase storage_capacity for regions with high net load variations",
            "3. Increase exchange_capacity to allow more energy transfer between regions",
            "4. Check data quality to ensure net load values are within reasonable ranges",
            "5. Consider introducing more gradual ramp-up/down constraints for dispatch",
            "6. If using a full year of data, try running the model on a smaller time period (e.g., one month) to isolate the issue"
        ]
        
        logger.info(f"Infeasibility analysis completed. Problematic regions: {slack_analysis['problematic_regions']}")
        for suggestion in slack_analysis["suggestions"]:
    ...
    def get_results(self) -> Dict:
        ...
        """
        # Check if we have regions available to extract results from
